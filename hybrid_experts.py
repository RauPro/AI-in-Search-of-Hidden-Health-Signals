...
# Waves where we measure features  →  outcome is wave + HORIZON
FEATURE_WAVES   = list(range(5, 13))   # 5..12
PREDICTION_HORIZON = 2                  # 2 waves = 4 years
LAGS = 2                               # use wave, wave-1, wave-2
...
# ═════════════════════════════════════════════════════════════════════
#  PHASE 1: EXTRACT FEATURES PER WAVE (with lags)
# ═════════════════════════════════════════════════════════════════════

def extract_wave_features(df_raw: pd.DataFrame, wave: int) -> pd.DataFrame:
    """
    For a given wave, extract:
      - Demographics (static)
      - Current wave values
    ...
    age = WAVE_YEARS[wave] - birth_year
    data["birth_year"] = birth_year.values
    data["age"] = age.values
    data["age_squared"] = (age ** 2).values

    data["female"]    = (pd.to_numeric(df_raw["RAGENDER"], errors="coerce") == 2).astype(int).values
    data["white"]     = (pd.to_numeric(df_raw["RARACEM"], errors="coerce") == 1).astype(int).values
    data["black"]     = (pd.to_numeric(df_raw["RARACEM"], errors="coerce") == 2).astype(int).values
    data["hispanic"]  = (pd.to_numeric(df_raw["RAHISPAN"], errors="coerce") == 1).astype(int).values
    ...
    for lag in range(LAGS + 1):
        ...
        for name, code in WAVE_VARS.items():
            col = f"R{w}{code}"
...
def add_temporal_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ── BMI trajectories ──
    new["bmi_velocity_2yr"]  = (df.get("bmi", np.nan) - df.get("bmi_lag1", np.nan)) / 2
    new["bmi_velocity_4yr"]  = (df.get("bmi", np.nan) - df.get("bmi_lag2", np.nan)) / 4
    new["bmi_accel"]         = new["bmi_velocity_2yr"] - (
        (df.get("bmi_lag1", np.nan) - df.get("bmi_lag2", np.nan)) / 2
    )
    ...
    new["rapid_weight_gain"] = (new["bmi_velocity_2yr"] > 1).astype(int)
    new["rapid_weight_loss"] = (new["bmi_velocity_2yr"] < -1).astype(int)

    # ── Health perception ──
    new["health_decline_2yr"] = df.get("self_rated_health", np.nan) - df.get("self_rated_health_lag1", np.nan)
    new["health_decline_4yr"] = df.get("self_rated_health", np.nan) - df.get("self_rated_health_lag2", np.nan)
    new["health_worsening"]   = (new["health_decline_2yr"] > 0).astype(int)
    new["health_crash"]       = (new["health_decline_2yr"] >= 2).astype(int)

    # ── Functional decline ──
    for base in ["mobility", "adl", "iadl"]:
        cur = df.get(base, np.nan)
        lag1 = df.get(f"{base}_lag1", np.nan)
        new[f"{base}_decline_2yr"] = cur - lag1
        new[f"{base}_worsening"]   = (new[f"{base}_decline_2yr"] > 0).astype(int)
    ...
    new["cog_decline_2yr"]     = cog_l1 - cog
    new["cog_decline_4yr"]     = cog_l2 - cog
    new["cog_worsening"]       = (new["cog_decline_2yr"] > 0).astype(int)
    new["sharp_cog_drop"]      = (new["cog_decline_2yr"] > 3).astype(int)
    new["low_cognition"]       = (cog < 12).astype(int)

    mem    = df.get("memory_recall", np.nan)
    mem_l1 = df.get("memory_recall_lag1", np.nan)
    new["memory_decline_2yr"]  = mem_l1 - mem
    new["memory_worsening"]    = (new["memory_decline_2yr"] > 0).astype(int)
    ...
    new["cog_decline_x_age"]     = new["cog_decline_2yr"] * df.get("age", np.nan)
...
def create_targets(df_raw: pd.DataFrame, feature_wave: int, outcome_wave: int) -> pd.DataFrame:
    """
    For each disease, create:
      - target_{disease}: 1 if onset (0→1) between feature_wave and outcome_wave
      - eligible_{disease}: 1 if disease=0 at feature_wave
    """
    ...
    for disease_name, code in DISEASE_MAP.items():
        ...
        if baseline_col in df_raw.columns and outcome_col in df_raw.columns:
            ...
            no_disease = (baseline == 0)
            develops   = (outcome == 1)

            target_vals = (no_disease & develops).astype(float)
            target_vals[outcome.isna()] = np.nan
...
def build_master_features(df_raw: pd.DataFrame) -> pd.DataFrame:
    """Build multi-wave long-format features + targets."""
    ...
    for wave in FEATURE_WAVES:
        ...
        # Features for this wave (with lags)
        feats = extract_wave_features(df_raw, wave)
        feats = add_temporal_features(feats)

        # Targets from this wave → outcome wave
        targs = create_targets(df_raw, wave, outcome_wave)

        # Merge on person_id
        combined = feats.merge(targs, on="person_id", how="inner")
        combined["feature_wave"] = wave
        combined["outcome_wave"] = outcome_wave
        all_data.append(combined)

        print(f"  Wave {wave} → {outcome_wave}: {len(combined):,} rows, "
...
def get_disease_dataset(master: pd.DataFrame, disease: str):
    ...
    metadata_cols = {"person_id", "feature_wave", "outcome_wave"}
    # Also exclude demographic columns used for fairness audit
    fairness_cols = {"female", "white", "black", "hispanic"}
    ...
    demo_test = sub.iloc[test_idx][["person_id", "female", "age"]].copy()
...
def build_any_onset_from_experts(
    ...
    # Get all test-set person-wave combinations that had at least one expert
    # For simplicity, we rebuild predictions disease by disease
    # and combine per person-wave

    person_waves = master[["person_id", "feature_wave"]].drop_duplicates()
    ...
    for disease, models in all_models.items():
        ...
        for i, (pid, fw) in enumerate(zip(sub["person_id"].values, sub["feature_wave"].values)):
            key = (pid, fw)
            if key not in results_per_pw:
                results_per_pw[key] = {"probs": [], "has_target": 0, "any_eligible": False}
            results_per_pw[key]["probs"].append(avg_pred[i])
            results_per_pw[key]["any_eligible"] = True
            # Any-onset target: did they develop ANY disease?
            if sub.iloc[i][target_col] == 1:
                results_per_pw[key]["has_target"] = 1

    # Compute P(any onset) = 1 - prod(1 - p_d)
    y_true_list = []
    y_proba_list = []

    for key, info in results_per_pw.items():
...
def fairness_audit(disease, y_test, y_proba, threshold, demo_test):
    ...
    for col, groups in [("female", {0: "Male", 1: "Female"})]:
        if col not in demo_test.columns:
            continue
        for gval, gname in groups.items():
            mask = demo_test[col].values == gval
            if mask.sum() < 50:
                continue
            ...
            print(f"    {gname:<8}: F2={g_f2:.4f}  P={g_prec:.4f}  R={g_rec:.4f} (n={mask.sum():,})")
...
def main():
    ...
    print(f"  Horizon: {PREDICTION_HORIZON} waves ({PREDICTION_HORIZON*2} years)")
